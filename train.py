...
if __name__ == "__main__":
    ...
    optimizer = NoamOpt(
        state_size, 2, 8000, torch.optim.Adam(model.parameters(), lr=0, betas=(0.9, 0.998), eps=1e-9)
    )
    cross_entropy = nn.NLLLoss(ignore_index=target_vocab.PAD_INDEX, reduction="sum")
    ...
    for epoch in range(1, 1001):
        ...
        with torch.no_grad():
            for batch in valid_data_loader:
                ...
                premises = y[:, 1:]
                premises = premises[
                    (premises != target_vocab.PAD_INDEX) & (premises != target_vocab.START_END_INDEX)
                ]
                output = model(source=x, type="beam_search", beam_size=10, max_length=64)
                output = output[
                    (output != target_vocab.PAD_INDEX) & (output != target_vocab.START_END_INDEX)
                ].unique()
                union_size, intersection_size, num_premises = get_union_and_intersection_size(
                    output, premises
                )
