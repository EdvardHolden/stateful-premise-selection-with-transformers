...
jaccard_indices = [jaccard_index(deps_predicted[thm], deps_true[thm]) for thm in set(deps_predicted)]
avg_jaccard_index = sum(jaccard_indices) / len(jaccard_indices)

coverages = [coverage(deps_predicted[thm], deps_true[thm]) for thm in set(deps_predicted)]
avg_coverage = sum(coverages) / len(coverages)

total_jaccard = sum([len(deps_predicted[thm] & deps_true[thm]) for thm in deps_predicted]) / sum(
    [len(deps_predicted[thm] | deps_true[thm]) for thm in deps_true]
)
total_coverage = sum([len(deps_predicted[thm] & deps_true[thm]) for thm in deps_predicted]) / sum(
    [len(deps_true[thm]) for thm in deps_true]
)
print("Average Jaccard index: {0:.3f}".format(avg_jaccard_index))
print("Average coverage: {0:.3f}".format(avg_coverage))
# print('Total Jaccard: {0:.3f}'.format(total_jaccard))
# print('Total coverage: {0:.3f}'.format(total_coverage))
...
if args.heights:
    ...
    for i in range(max_height):
        ...
        if thms_h:
            jaccard_indices = [jaccard_index(deps_predicted[thm], deps_true[thm]) for thm in thms_h]
            avg_jaccard_index = sum(jaccard_indices) / len(jaccard_indices)

            coverages = [coverage(deps_predicted[thm], deps_true[thm]) for thm in thms_h]
            avg_coverage = sum(coverages) / len(coverages)
