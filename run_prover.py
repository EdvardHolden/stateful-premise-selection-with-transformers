...
def find_proof(conj, deps, statements_file, dir_path, logger, format_2):
    ...
    input_filename = problem_file(conj, deps, statements_file, dir_path, format_2)
    ...
    if "# Proof found!" in read_lines(output_filename):
        logger.print("PROVED#" + conj + ":" + " ".join(deps) + "#Output: " + output_filename)
        return True
    else:
        logger.print("NOT proved#" + conj + ":" + " ".join(deps) + "#Output: " + output_filename)
        return False
...
def atp_eval(deps_file, statements_file, dir_path=None, logger=None, format_2=False, n_jobs=-1):
    deps = read_deps(deps_file)
    with Parallel(n_jobs=n_jobs) as parallel:
        dfp = delayed(find_proof_wrapper)
        proven = parallel(dfp(c, ds, statements_file, dir_path, logger, format_2) for c, ds in deps)
    return proven
...
atp_output = atp_eval(args.dependencies, args.statements, dir_path, logger, args.format_2)
...
logger.print(f"Number of successful attempts: " f"{n_succ_attempts} ({prop_succ_attempts})")
logger.print(f"Number of proved conjectures: " f"{n_proved_conjs} ({prop_proved_conjs})")
